# Flagd (Feature Flags)

> **Mentor note:** Study this file with the source tree open. Diagrams first, then code, then YAML.  
> **Shared YAML deep-dive:** [_KUBERNETES_YAML_HELM_ARGOCD.md](./_KUBERNETES_YAML_HELM_ARGOCD.md) · **Map:** [_SERVICE_MAP.md](./_SERVICE_MAP.md) · **Index:** [README.md](./README.md)

---

## 1. Why this service exists

Open Feature flag evaluation for chaos/demo scenarios.

| | |
|--|--|
| **Language** | flagd + Next.js UI |
| **Source** | `src/flagd, src/flagd-ui/` |
| **Entry** | `demo.flagd.json, flagd-ui` |
| **K8s folder** | `kubernetes/flagd/` |
| **Container name** | `flagd (+ flagdui sidecar)` |
| **Protocol** | gRPC :8013 / UI :4000 |
| **Docker port** | 8013 |
| **K8s port** | 8013 |

---

## 2. Where it sits in the architecture

```mermaid
flowchart LR
  S["flagd"]
  ad["ad"] --> S
  cart["cart"] --> S
  checkout["checkout"] --> S
  frontend["frontend"] --> S
  fraud["fraud"] --> S
  payment["payment"] --> S
  product-catalog["product-catalog"] --> S
  recommendation["recommendation"] --> S
  load-generator["load-generator"] --> S
  frontend-proxy["frontend-proxy"] --> S
  S --> out_otel["otel"]
```

### Callers / callees

| Direction | Services |
|-----------|----------|
| **Who calls me** | `ad`, `cart`, `checkout`, `frontend`, `fraud`, `payment`, `product-catalog`, `recommendation`, `load-generator`, `frontend-proxy` |
| **Who I call** | `otel` |

---

## 3. Source code architecture (how to read the code)

1. Open `src/flagd, src/flagd-ui/` and locate `demo.flagd.json, flagd-ui`.
2. Find listen/bind port (env `*_PORT` or hardcoded) — in Docker often **8013**, in K8s usually **8013**.
3. Find outbound clients (gRPC stubs, HTTP, Kafka, Redis) matching the callees table.
4. Find OpenTelemetry setup (`OTEL_*` env, auto-instrumentation, or SDK init).
5. Shared API contracts live in `pb/demo.proto` for gRPC services.

```mermaid
flowchart TB
  subgraph Code["src/flagd, src/flagd-ui"]
    Main["demo.flagd.json, flagd-ui"]
    Biz[Business logic]
    Client[Outbound clients]
    OTel[Telemetry hooks]
  end
  Main --> Biz
  Biz --> Client
  Main --> OTel
  Client --> Net["Network: gRPC :8013 / UI :4000"]
  OTel --> Collector[OTEL collector endpoint]
```

---

## 4. Request scenario

**Service evaluates flag via FLAGD_HOST:FLAGD_PORT → may inject latency/error.**

```mermaid
sequenceDiagram
  participant Caller as Caller
  participant S as flagd
  participant Dep as Downstream
  Caller->>S: Request (gRPC :8013 / UI :4000)
  S->>Dep: Calls (if any)
  Dep-->>S: Response
  S-->>Caller: Response
```

---

## 5. Kubernetes: how this service is deployed

```mermaid
flowchart TB
  subgraph Git["Git: kubernetes/flagd/"]
    D[deploy.yaml]
    SV[svc.yaml]
  end
  subgraph Cluster["EKS namespace otel-demo"]
    Pod["Pod container: flagd (+ flagdui sidecar)"]
    Svc["Service ClusterIP :8013"]
  end
  D -->|Argo CD sync| Pod
  SV -->|Argo CD sync| Svc
  Svc -->|selects labels| Pod
```

### Files

| File | Purpose |
|------|---------|
| `kubernetes/flagd/deploy.yaml` | Deployment (Pods) |
| `kubernetes/flagd/svc.yaml` | ClusterIP Service |

### Deployment essentials (read `deploy.yaml`)

| Field | This service |
|-------|----------------|
| `metadata.name` | `opentelemetry-demo-flagd (+ flagdui sidecar)` (typical) |
| `spec.replicas` | Usually `1` |
| `spec.selector` / pod labels | Must match Service selector |
| `containers[].name` | `flagd (+ flagdui sidecar)` |
| `containers[].image` | CI sets `DOCKER_USERNAME/flagd:<run_id>` (or upstream `ghcr.io/...`) |
| `containerPort` | 8013 |
| `initContainers` | Yes — wait for dependency (see YAML) |
| `serviceAccountName` | `opentelemetry-demo` |

### Environment variables present in deploy.yaml

| Env var | Notes |
|---------|-------|
| `OTEL_SERVICE_NAME` | See deploy.yaml / shared OTEL guide |
| `OTEL_COLLECTOR_NAME` | See deploy.yaml / shared OTEL guide |
| `OTEL_EXPORTER_OTLP_METRICS_TEMPORALITY_PREFERENCE` | See deploy.yaml / shared OTEL guide |
| `FLAGD_METRICS_EXPORTER` | See deploy.yaml / shared OTEL guide |
| `FLAGD_OTEL_COLLECTOR_URI` | See deploy.yaml / shared OTEL guide |
| `OTEL_RESOURCE_ATTRIBUTES` | See deploy.yaml / shared OTEL guide |
| `OTEL_SERVICE_NAME` | See deploy.yaml / shared OTEL guide |
| `OTEL_COLLECTOR_NAME` | See deploy.yaml / shared OTEL guide |
| `OTEL_EXPORTER_OTLP_METRICS_TEMPORALITY_PREFERENCE` | See deploy.yaml / shared OTEL guide |
| `FLAGD_METRICS_EXPORTER` | See deploy.yaml / shared OTEL guide |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | See deploy.yaml / shared OTEL guide |
| `OTEL_RESOURCE_ATTRIBUTES` | See deploy.yaml / shared OTEL guide |

Boilerplate `OTEL_*` meaning: see [_KUBERNETES_YAML_HELM_ARGOCD.md](./_KUBERNETES_YAML_HELM_ARGOCD.md).

### Service (ClusterIP) — if present

```yaml\n# kubernetes/flagd/svc.yaml — key ideas:\n# type: ClusterIP\n# port/targetPort: 8013\n# selector: opentelemetry.io/name: opentelemetry-demo-flagd (+ flagdui sidecar)\n```

### DNS name used by other services

```text
opentelemetry-demo-flagd (+ flagdui sidecar):8013
```

Example from another Deployment env: `PRODUCT_CATALOG_SERVICE_ADDR` / `CART_SERVICE_ADDR` style values use `opentelemetry-demo-<component>:8080`.

---

## 6. GitOps / CI for this service

| | |
|--|--|
| **CI workflow** | UI image may be upstream; flagd uses ghcr.io/open-feature/flagd |
| **Image update** | reusable job patches `image:` for container `flagd (+ flagdui sidecar)` in `deploy.yaml` |
| **Deploy** | Argo CD Application `otel-demo` syncs `kubernetes/` (excludes `complete-deploy.yaml`) |

```mermaid
flowchart LR
  Code[src change] --> GHA[GitHub Actions]
  GHA --> Hub[Docker Hub]
  GHA --> Git[Update deploy.yaml]
  Git --> Argo[Argo CD]
  Argo --> EKS[EKS Pod]
```

---

## 7. Interview talking points

- Role: Open Feature flag evaluation for chaos/demo scenarios.
- Protocol: gRPC :8013 / UI :4000 — Docker port 8013 vs K8s 8013.
- Dependencies: callers `ad, cart, checkout, frontend, fraud, payment, product-catalog, recommendation, load-generator, frontend-proxy`; callees `otel`.
- Manifests: `kubernetes/flagd/` — has Service.
- Discovery: Kubernetes DNS `opentelemetry-demo-flagd (+ flagdui sidecar):8013`.
- Observability: `OTEL_EXPORTER_OTLP_ENDPOINT` points at collector Service name.
- GitOps: CI never runs `kubectl apply`; it only updates Git for Argo.
- Chaos/demo: many services use `FLAGD_HOST` / `FLAGD_PORT` for Open Feature.

---

## 8. Quick quiz

**Q1.** Who calls `flagd` in the shop?  
**A:** ad, cart, checkout, frontend, fraud, payment, product-catalog, recommendation, load-generator, frontend-proxy.

**Q2.** What Kubernetes DNS would another Pod use (if any)?  
**A:** `opentelemetry-demo-flagd (+ flagdui sidecar):8013`.

**Q3.** Does Argo deploy from `complete-deploy.yaml` or per-service folders?  
**A:** Per-service folders under `kubernetes/`; `complete-deploy.yaml` is excluded.

---

## 9. Related reading

- [README.md](./README.md) — learning path  
- [_SERVICE_MAP.md](./_SERVICE_MAP.md) — place-order sequence  
- [_KUBERNETES_YAML_HELM_ARGOCD.md](./_KUBERNETES_YAML_HELM_ARGOCD.md) — YAML line-by-line  
- [../INTERVIEW_QUESTIONS.md](../INTERVIEW_QUESTIONS.md)  
