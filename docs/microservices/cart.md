...
Stores cart items in Valkey (Redis-compatible).

| | |
|--|--|
| **Language** | C# / .NET 8 |
| **Source** | `src/cart/` |
| **Entry** | `src/Program.cs, src/services/CartService.cs` |
| **K8s folder** | `kubernetes/cart/` |
...
| **Who I call** | `valkey`, `flagd`, `otel` |

---

## 3. Source code architecture (how to read the code)

1. Open `src/cart/` and locate `src/Program.cs, src/services/CartService.cs`.
2. Find listen/bind port (env `*_PORT` or hardcoded) — in Docker often **7070**, in K8s usually **8080**.
...
```mermaid
flowchart TB
  subgraph Code["src/cart"]
    Main["src/Program.cs, src/services/CartService.cs"]
    Biz[Business logic]
...
**Add to cart → frontend/checkout → cart → Valkey.**

```mermaid
sequenceDiagram
  participant Caller as Caller
  participant S as cart
...
```mermaid
flowchart TB
  subgraph Git["Git: kubernetes/cart/"]
    D[deploy.yaml]
    SV[svc.yaml]
  end
  subgraph Cluster["EKS namespace otel-demo"]
    Pod["Pod container: cartservice"]
...
| File | Purpose |
|------|---------|
| `kubernetes/cart/deploy.yaml` | Deployment (Pods) |
| `kubernetes/cart/svc.yaml` | ClusterIP Service |

### Deployment essentials (read `deploy.yaml`)

| Field | This service |
|-------|----------------|
| `metadata.name` | `opentelemetry-demo-cartservice` (typical) |
| `spec.replicas` | Usually `1` |
| `spec.selector` / pod labels | Must match Service selector |
| `containers[].name` | `cartservice` |
| `containers[].image` | CI sets `DOCKER_USERNAME/cart:<run_id>` (or upstream `ghcr.io/...`) |
| `containerPort` | 8080 |
...
| `CART_SERVICE_PORT` | See deploy.yaml / shared OTEL guide |
| `ASPNETCORE_URLS` | See deploy.yaml / shared OTEL guide |
| `VALKEY_ADDR` | See deploy.yaml / shared OTEL guide |
...
```yaml\n# kubernetes/cart/svc.yaml — key ideas:\n# type: ClusterIP\n# port/targetPort: 8080\n# selector: opentelemetry.io/name: opentelemetry-demo-cartservice\n```
...
| | |
|--|--|
| **CI workflow** | microservices-ci (context: repo root, Dockerfile src/cart/src) |
| **Image update** | reusable job patches `image:` for container `cartservice` in `deploy.yaml` |
...
- Role: Stores cart items in Valkey (Redis-compatible).
- Protocol: gRPC — Docker port 7070 vs K8s 8080.
- Dependencies: callers `frontend, checkout`; callees `valkey, flagd, otel`.
- Manifests: `kubernetes/cart/` — has Service.
- Discovery: Kubernetes DNS `opentelemetry-demo-cartservice:8080`.
...
**Q1.** Who calls `cart` in the shop?  
**A:** frontend, checkout.

**Q2.** What Kubernetes DNS would another Pod use (if any)?  
**A:** `opentelemetry-demo-cartservice:8080`.
