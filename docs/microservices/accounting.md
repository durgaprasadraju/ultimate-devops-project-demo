...
Kafka consumer for order events (no public Service).

| | |
|--|--|
| **Language** | C# / .NET |
| **Source** | `src/accounting/` |
| **Entry** | `Program.cs, Consumer.cs` |
| **K8s folder** | `kubernetes/accounting/` |
| **Container name** | `accountingservice` |
...
| **Who I call** | `kafka`, `otel` |

---

## 3. Source code architecture (how to read the code)

1. Open `src/accounting/` and locate `Program.cs, Consumer.cs`.
...
```mermaid
flowchart TB
  subgraph Code["src/accounting"]
    Main["Program.cs, Consumer.cs"]
    Biz[Business logic]
...
**checkout publishes → Kafka → accounting consumes.**

```mermaid
sequenceDiagram
  participant Caller as Caller
  participant S as accounting
...
```mermaid
flowchart TB
  subgraph Git["Git: kubernetes/accounting/"]
    D[deploy.yaml]
    SV[svc.yaml]
  end
  subgraph Cluster["EKS namespace otel-demo"]
    Pod["Pod container: accountingservice"]
...
| File | Purpose |
|------|---------|
| `kubernetes/accounting/deploy.yaml` | Deployment (Pods) |
| `kubernetes/accounting/svc.yaml` | **None** — no ClusterIP (worker/consumer or special case) |

### Deployment essentials (read `deploy.yaml`)

| Field | This service |
|-------|----------------|
| `metadata.name` | `opentelemetry-demo-accountingservice` (typical) |
| `spec.replicas` | Usually `1` |
| `spec.selector` / pod labels | Must match Service selector |
| `containers[].name` | `accountingservice` |
| `containers[].image` | CI sets `DOCKER_USERNAME/accounting:<run_id>` (or upstream `ghcr.io/...`) |
| `containerPort` | n/a |
...
| **Image update** | reusable job patches `image:` for container `accountingservice` in `deploy.yaml` |
...
- Role: Kafka consumer for order events (no public Service).
- Protocol: Kafka consumer — Docker port n/a vs K8s n/a.
- Dependencies: callers `kafka (orders topic)`; callees `kafka, otel`.
- Manifests: `kubernetes/accounting/` — no Service (consumer/worker).
...
**Q1.** Who calls `accounting` in the shop?  
